...
public interface EmpregadoRepository extends JpaRepository<Empregado, Integer> {
    ...
    @Query(value = "SELECT * FROM usuarios INNER JOIN cargo ON usuarios.id_cargo = cargo.id_cargo WHERE dtype = 'Empregado' AND cargo.salario BETWEEN :valor1 AND :valor2", nativeQuery = true)
